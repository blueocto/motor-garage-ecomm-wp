...
function commercekit_ajax_save_wishlist() {
	...
	if ( $pid ) {
		...
		$wsl_adtext     = isset( $commercekit_options['wsl_adtext'] ) && ! empty( $commercekit_options['wsl_adtext'] ) ? esc_attr( stripslashes_deep( $commercekit_options['wsl_adtext'] ) ) : esc_html__( 'Add to wishlist', 'commercegurus-commercekit' );
		$wsl_brtext     = isset( $commercekit_options['wsl_brtext'] ) && ! empty( $commercekit_options['wsl_brtext'] ) ? esc_attr( stripslashes_deep( $commercekit_options['wsl_brtext'] ) ) : esc_html__( 'Browse wishlist', 'commercegurus-commercekit' );
		$wsl_pdtext     = isset( $commercekit_options['wsl_pdtext'] ) && ! empty( $commercekit_options['wsl_pdtext'] ) ? esc_attr( stripslashes_deep( $commercekit_options['wsl_pdtext'] ) ) : esc_html__( 'Product added', 'commercegurus-commercekit' );
...
function commercekit_ajax_remove_wishlist() {
	...
	$ajax['status'] = 1;
	$ajax['html']   = '<a href="javascript:;" class="commercekit-save-wishlist" data-product-id="' . $pid . '" data-type="list"><i class="cg-wishlist"></i><span></span></a>';
...
add_action( 'woocommerce_after_shop_loop_item', 'commercekit_after_shop_loop_item_wishlist', 15 );
...
function commercekit_shortcode_wishlist() {
	...
	<table class="commercekit-wishlist-table" width="100%">
		<thead>
			<tr>
				<th id="cb">&nbsp;</th>
				<th id="product"><?php esc_html_e( 'Product', 'commercegurus-commercekit' ); ?></th>
		...
		<tbody>
			...
			<tr id="product-<?php echo esc_attr( $row['product_id'] ); ?>">
				<td class="remove">
					<a href="javascript:;" class="commercekit-remove-wishlist2" data-product-id="<?php echo esc_html( $row['product_id'] ); ?>" data-wpage="<?php echo esc_html( $wpage ); ?>">×</a>
				</td>
				<td class="image-name">
					<div class="image">
